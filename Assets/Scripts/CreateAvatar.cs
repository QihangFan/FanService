...
public class CreateAvatar : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        controls.GamePlay.MaleSpawn.performed += ctx => spawnMale = ctx.ReadValue<float>(); //Y
        controls.GamePlay.MaleSpawn.canceled += ctx => spawnMale = 0f;

        controls.GamePlay.FemaleSpawn.performed += ctx => spawnFemale = ctx.ReadValue<float>(); //X
        ...
        controls.GamePlay.AnimationOne.performed += ctx => animationOne = ctx.ReadValue<float>(); //B
    ...
    private void maleSpawn()
    {
        ...
        //reset meshindex
        meshIndex = (int)UnityEngine.Random.Range(0, meshNumber * 0.3f);
    ...
    void Update()
    {
        ...
        if(gameState == -1)
        {
            ...
            timerAndScore.text = null;
            startInstruction.text = "Press B to start";
            if (animationOne != 0)
            {
                gameState = 0;
            }
        ...
        else if (gameState == 0)
        {
            ...
            startInstruction.text = "Press Y to spawn a male avatar" + "\n" + "Press X to spawn a female avatar";
